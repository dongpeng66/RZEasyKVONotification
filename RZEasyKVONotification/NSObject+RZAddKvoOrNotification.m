//
//  NSObject+RZAddKvoOrNotification.m
...
@interface _RZBlockTarget : NSObject

/**添加一个KVOBlock*/
- (void)rz_addBlock:(void(^)(__weak id obj, id oldValue, id newValue))block;
- (void)rz_addNotificationBlock:(void(^)(NSNotification *notification))block;

- (void)rz_doNotification:(NSNotification*)notification;

@end

@implementation _RZBlockTarget{
    //保存所有的block
...
@implementation NSObject (RZAddKvoOrNotification)
static void *const RZKVOBlockKey = "RZKVOBlockKey";

- (void)rz_addObserverBlockForKeyPath:(NSString*)keyPath block:(void (^)(id obj, id oldVal, id newVal))block {
    if (!keyPath || !block) return;
    //取出存有所有KVOTarget的字典
    NSMutableDictionary *allTargets = objc_getAssociatedObject(self, RZKVOBlockKey);
    if (!allTargets) {
        //没有则创建
        allTargets = [NSMutableDictionary new];
        //绑定在该对象中
        objc_setAssociatedObject(self, RZKVOBlockKey, allTargets, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    //获取对应keyPath中的所有target
    _RZBlockTarget *targetForKeyPath = allTargets[keyPath];
    if (!targetForKeyPath) {
        //没有则创建
        targetForKeyPath = [_RZBlockTarget new];
        //保存
    ...
    [targetForKeyPath rz_addBlock:block];
    //对第一次注册KVO的类进行dealloc方法调剂
    [self _rz_swizzleDealloc];
}
- (void)rz_removeObserverBlockForKeyPath:(NSString *)keyPath{
    if (!keyPath.length) return;
    NSMutableDictionary *allTargets = objc_getAssociatedObject(self, RZKVOBlockKey);
    if (!allTargets) return;
    _RZBlockTarget *target = allTargets[keyPath];
    if (!target) return;
    [self removeObserver:target forKeyPath:keyPath];
    [allTargets removeObjectForKey:keyPath];
}

- (void)rz_removeAllObserverBlocks {
    NSMutableDictionary *allTargets = objc_getAssociatedObject(self, RZKVOBlockKey);
    if (!allTargets) return;
    [allTargets enumerateKeysAndObjectsUsingBlock:^(id key, _RZBlockTarget *target, BOOL *stop) {
        [self removeObserver:target forKeyPath:key];
    }];
    [allTargets removeAllObjects];
}

static void *const RZNotificationBlockKey = "RZNotificationBlockKey";

- (void)rz_addNotificationForName:(NSString *)name block:(void (^)(NSNotification *notification))block {
    if (!name || !block) return;
    NSMutableDictionary *allTargets = objc_getAssociatedObject(self, RZNotificationBlockKey);
    if (!allTargets) {
        allTargets = @{}.mutableCopy;
        objc_setAssociatedObject(self, RZNotificationBlockKey, allTargets, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    _RZBlockTarget *target = allTargets[name];
    if (!target) {
        target = [_RZBlockTarget new];
        allTargets[name] = target;
        [[NSNotificationCenter defaultCenter] addObserver:target selector:@selector(rz_doNotification:) name:name object:nil];
    }
    [target rz_addNotificationBlock:block];
    [self _rz_swizzleDealloc];
    
}

- (void)rz_removeNotificationForName:(NSString *)name{
    if (!name) return;
    NSMutableDictionary *allTargets = objc_getAssociatedObject(self, RZNotificationBlockKey);
...
- (void)rz_removeAllNotification{
    NSMutableDictionary *allTargets = objc_getAssociatedObject(self, RZNotificationBlockKey);
    if (!allTargets.count) return;
    [allTargets enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, _RZBlockTarget *target, BOOL * _Nonnull stop) {
        [[NSNotificationCenter defaultCenter] removeObserver:target];
    }];
    [allTargets removeAllObjects];
}

- (void)rz_postNotificationWithName:(NSString *)name userInfo:(NSDictionary *)userInfo{
...
- (void)_rz_swizzleDealloc{
    ...
    id newDealloc = ^(__unsafe_unretained id objSelf){
        //在这里我们移除所有的KVO
        [objSelf rz_removeAllObserverBlocks];
        //移除所有通知
        [objSelf rz_removeAllNotification];
